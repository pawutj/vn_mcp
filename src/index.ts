...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (request.params.name === "search_visual_novels") {
    ...
    const results = vn_list.filter( (vn:any) => {
  ...
  if (request.params.name === "get_visual_novel_details") {
    ...
    const vn = vn_list.find((v:any) => v.name.toLowerCase() === name.toLowerCase());
    
    if (!vn) {
      // If not found by exact match, try to find by partial match
      const partialMatch = vn_list.find((v:any) => v.name.toLowerCase().includes(name.toLowerCase()));
      if (partialMatch) {
  ...
  if (request.params.name === "search_visual_novels_by_tags") {
    const args = request.params.arguments as { tags: string[] };
    const { tags } = args;
    ...
    const results = vn_list.filter((vn:any) => {
      ...
      const vnTags = vn.vntags[0].map((tag: string) => tag.toLowerCase());
      
      // Check if all search tags are present in the VN's tags
      return searchTags.every(searchTag => {
        return vnTags.some( (vnTag:any) => vnTag.includes(searchTag));
      });
    }).map( (vn:any) => ({
      name: vn.name,
      url: vn.url,
